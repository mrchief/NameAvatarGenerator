...
namespace NameAvatarGenerator.ColorHelper
{
    ...
    public static class ColorSpaceHelper
    {
        ...
        public static Color[] GetWheelColors(int alpha)
        {
            ...
            {
                var temp = HSBtoColor((int) (i*255.0/colorCount), 255, 255);
                colors[i] = Color.FromArgb((byte) alpha, temp.R, temp.G, temp.B);
            }
        ...
        public static Color[] GetSpectrumColors(byte alpha)
        {
            var colors = new Color[256*6];
            //for(int i=127; i<256; i++) colors[i-127] = Color.FromArgb(alpha, i, 0, 0);
            for (var i = 0; i <= 255; i++) colors[i] = Color.FromArgb(alpha, 255, (byte) i, 0);
            for (var i = 0; i < 256; i++) colors[i + (256)] = Color.FromArgb(alpha, (byte) (255 - i), 255, 0);
            for (var i = 0; i < 256; i++) colors[i + (256*2)] = Color.FromArgb(alpha, 0, 255, (byte) i);
            for (var i = 0; i < 256; i++) colors[i + (256*3)] = Color.FromArgb(alpha, 0, (byte) (255 - i), 255);
            for (var i = 0; i < 256; i++) colors[i + (256*4)] = Color.FromArgb(alpha, (byte) i, 0, 255);
            for (var i = 0; i < 256; i++) colors[i + (256*5)] = Color.FromArgb(alpha, 255, 0, (byte) (255 - i));
        ...
        public static CIEXYZ RGBtoXYZ(int red, int green, int blue)
        {
            ...
            var r = (rLinear > 0.04045) ? Math.Pow((rLinear + 0.055)/(1 + 0.055), 2.4) : (rLinear/12.92);
            var g = (gLinear > 0.04045) ? Math.Pow((gLinear + 0.055)/(1 + 0.055), 2.4) : (gLinear/12.92);
            var b = (bLinear > 0.04045) ? Math.Pow((bLinear + 0.055)/(1 + 0.055), 2.4) : (bLinear/12.92);
        ...
        public static CIELab XYZtoLab(double x, double y, double z)
        {
            ...
            // Changes from Rob2412
            //lab.L = 116.0 * Fxyz(y / CIEXYZ.D50.Y) - 16;
            //lab.A = 500.0 * (Fxyz(x / CIEXYZ.D50.X) - Fxyz(y / CIEXYZ.D50.Y));
            //lab.B = 200.0 * (Fxyz(y / CIEXYZ.D50.Y) - Fxyz(z / CIEXYZ.D50.Z));
        ...
        public static CIEXYZ LabtoXYZ(double l, double a, double b)
        {
            ...
            // From Rob2412 for use with D50 white 
            //return new CIEXYZ(
            //    (fx > theta)? CIEXYZ.D50.X * (fx*fx*fx) : (fx - 16.0/116.0)*3*(theta*theta)*CIEXYZ.D50.X,
            //    (fy > theta)? CIEXYZ.D50.Y * (fy*fy*fy) : (fy - 16.0/116.0)*3*(theta*theta)*CIEXYZ.D50.Y,
            //    (fz > theta)? CIEXYZ.D50.Z * (fz*fz*fz) : (fz - 16.0/116.0)*3*(theta*theta)*CIEXYZ.D50.Z
            //    );
        }
