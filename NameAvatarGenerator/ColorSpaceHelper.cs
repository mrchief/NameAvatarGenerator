...
namespace Devcorp.Controls.Design
{
	...
	public sealed class ColorSpaceHelper
	{
		...
		{
			RGB rgb1 = HSLtoRGB(hsl1);
			RGB rgb2 = HSLtoRGB(hsl2);
		...
		public static Color[] GetSpectrumColors(byte alpha)
		{
			...
			for(int i=0; i<=255; i++) colors[i] = Color.FromArgb(alpha, 255, (byte)i, 0);
            for (int i = 0; i < 256; i++) colors[i + (256)] = Color.FromArgb(alpha, (byte)(255 - i), 255, 0);
			for (int i = 0; i < 256; i++) colors[i + (256 * 2)] = Color.FromArgb(alpha, 0, 255, (byte)i);
            for (int i = 0; i < 256; i++) colors[i + (256 * 3)] = Color.FromArgb(alpha, 0, (byte)(255 - i), 255);
			for (int i = 0; i < 256; i++) colors[i + (256 * 4)] = Color.FromArgb(alpha, (byte)i, 0, 255);
            for (int i = 0; i < 256; i++) colors[i + (256 * 5)] = Color.FromArgb(alpha, 255, 0, (byte)(255 - i));
